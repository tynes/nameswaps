...
class InvPacket extends Packet {
  ...
  // TODO: fix this as each item isn't necessarily 36
  getSize() {
    let size = 0;
    size += encoding.sizeVarint(this.items.length);
...
class TipPacket extends GetTipPacket {
  constructor(hash, flags) {
    super(hash, flags);
...
class ProgamPacket extends Packet {
  constructor() {
    super();
  }
}
  ...
  read(br) {
    ...
    switch (this.message) {
      case types.SWAPPROOF:
        this.hash = br.readVarBytes(); // TODO: double check this one
        break;
      case types.PROGRAM:
        this.hash = br.readHash();
        break;
