...
class NameSwaps extends AsyncEmitter {
  ...
  async open() {
    ...
    let tip = await this.getTipEntry();
    if (!tip) {
      const genesis = Block.decode(this.network.genesisBlock);
      tip = ChainEntry.fromBlock(genesis);
    }
  ...
  /**
   * High level api that indexes the swap proof
   * and the name by block hash.
   */

  async addSwapProof(proof) {
    // to get the block hash, must get the name outpoint
    // and then getcoin and then check the program against
    // the address then get the height on the coin and get
    // block by height and make sure the coin is in the main
    // chain.
    //
    // index the proof by block
    // index the proof by name
  }
