...
class SwapsDB extends EventEmitter {
  ...
  async getProgram(hash, index) {
    ...
    const program = Program.decode(raw, {hash, index});
  ...
  async getPrograms() {
    return this.db.range({
      ...
      lte: layout.p.max(),
      parse: (key, value) => {
        const [hash, index] = layout.p.decode(key);
        return Program.decode(value, {hash, index});
      }
    });
  }
  ...
  async putProgramOutpointByBlock(blockhash, hash, index) {
    const key = layout.P.encode(blockhash, hash, index);
  ...
  async putSwapProofNameByBlock(hash, name) {
    const key = layout.S.encode(hash, name);
  ...
  async getSwapProofNamesByBlock(hash) {
    return this.db.keys({
      gte: layout.S.min(hash),
      lte: layout.S.max(hash),
      parse: (key) => {
        const [, name] = layout.S.decode(key);
        return name;
      }
    });
  }
