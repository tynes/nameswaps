 ...
 *
 * GET /nameswaps
 * GET /namewaps/address
 * GET /namewaps/address/:address
...
class HTTP extends Server {
  ...
  initRouter() {
    ...
    /**
     * Get Witness by Address.
     */

    this.get('/nameswaps/address/:address', async (req, res) => {
      const valid = Validator.fromRequest(req);
      const address = valid.str('address');

      enforce(address, 'Must pass address');

      const witness = await this.nameswaps.getAddrWitness(address);
      ...
      res.json(200, witness.toJSON());
    });

    /**
     * Add a new AddrWitness
     */

    this.post('/nameswaps/address', async (req, res) => {
      const valid = Validator.fromRequest(req);
      const address = valid.str('address');
      const witness = valid.str('witness');

      enforce(address, 'Must pass address.');
      enforce(witness, 'Must pass witness.');

      let witness;
      try {
        witness = await this.nameswaps.putWitness(address, witness);
    ...
    /**
     * Get all Names ready to name swap
     */

    this.get('/nameswaps/name', async (req, res) => {
      // method on nameswaps to get all names

    });

    /**
     * Get info on a name ready to name swap
     */
  ...
  handleAuth(socket) {
    socket.hook('watch witness', () => {
      socket.join('witness');
      return null;
    });

    socket.hook('unwatch witness', () => {
      socket.leave('witness');
      return null;
    });
  }
  ...
  initSockets() {
    this.relay.on('witness', (data) => {
      ...
      this.to('witness', 'new witness', data);
    });
  }
}
