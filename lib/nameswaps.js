...
class NameSwaps extends AsyncEmitter {
  ...
  async putSwapProof(name, proof) {
    ...
    try {
      return await this._putSwapProof(name, proof);
    } catch (e) {
      this.emit('error', e);
      return null;
    } finally {
  ...
  async _putSwapProof(name, proof) {
  ...
  async _deleteSwapProof(name) {
    return this.sdb.deleteSwapProof();
  }
  /**
   *
   */
  ...
  async putProgram(hash, index, program) {
    const unlock = await this.writeLock.lock();
    try {
      return await this._putProgram(hash, index, program);
    } catch (e) {
      this.emit('error', e);
      return null;
    } finally {
  ...
  async _putProgram(hash, index, program) {
    // validate the program here
  ...
  async deleteProgram(hash, index) {
    const unlock = await this.writeLock.lock();
    try {
      return await this._deleteProgram(hash, index);
    } catch (e) {
      this.emit('error', e);
      return null;
    } finally {
  ...
  async _deleteProgram(hash, index) {
