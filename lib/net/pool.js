...
class Pool extends EventEmitter {
  ...
  async sendGetSyncData(peer) {
    ...
    const start = await this.nameswaps.getTip();
    const end = this.chain.tip.hash;

    // TODO:
    // this is where you need to get
    // the nameswap tip

    peer.sendGetSyncData(start, end);
  }
  ...
  sendGetTip(peer) {
    ...
    // TODO: need to expect a tip message back
    // need to expect a tip back
  ...
  async handlePacket(peer, packet) {
    switch (packet.type) {
      ...
      case packetTypes.GETSYNCDATA:
        await this.handleGetSyncData(peer, packet);
        break;
  ...
  /**
   * Handle `tip` packet.
   * If the height less than or equal to the best height,
   * make sure that the tip is in the main chain. Add to
   * the banscore of a peer that sends a tip in the main
   * chain. Since this accesses the databse, protect this
   * from dos attacks using the sliding window.
   * Definitely do not touch the database if the height
   * is greater than the known height.
   *
   * There is a race condition with reorgs, that should
   * rarely happen.
   */

  async handleTip(peer, packet) {
    if (packet.height <= this.nameswaps.tip.height) {
      if (!(await this.chain.isMainHash(packet.hash))) {
        //peer.SendReject(code, reason, msg, hash);
        peer.increaseBan(10);
      }
    }
  ...
  async handleGetProgram(peer, packet) {
    ...
    const program = await this.nameswaps.getProgram(packet.hash, packet.index);
    ...
    peer.send(new packets.ProgramPacket(program, packet.hash, packet.index));
  }
  ...
  async handleGetDataSync(peer, packet) {
    ...
    const packet = new packets.DataSyncPacket();
  ...
  async handleProgramInv(peer, programs) {
    ...
    this.getPrograms(peer, programs);
  }

  async handleSwapProofInv(peer, program) {
    // check to make sure its well formed
    // signature, value, data
    // make sure the name utxo exists
    // recreate the tx and validate the signature
    // index
  }
