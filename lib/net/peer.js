...
class Peer extends EventEmitter {
  ...
  sendGetTip() {
    const packet = new packets.GetTipPacket();

    this.logger.debug(
      'Requesting tip packet from peer with gettip (%s).',
      this.hostname());

    this.logger.debug('Sending gettip (hash=%x).');

    this.send(packet);
  }

  sendTip(tip) {
    const packet = new packets.TipPacket(tip);

    this.logger.debug('Sending tippacket (tip=%x)', tip);

    this.send(packet);
  }

  sendGetProgram(hash, index) {
    const packet = new packets.GetProgramPacket(hash, index);

    this.logger.debug('Sending getprogram (outpoint=%x/%s)', hash, index);

    this.send(packet);
  }

  sendGetSwapProof(name) {
    const packet = new packets.GetSwapProofPacket(name);

    this.logger.debug('Sending getswapproof (name=%s)', name);

    this.send(packet);
  }

  sendGetDataSync(locator, stop, flags) {
    const packet = new packets.GetDataSyncPacket(locator, stop, flags);

    // TODO: flags to array helper
    this.logger.debug('Sending getdatasync (start=%x, start=%x, flags=%s)', locator, stop, flags);
