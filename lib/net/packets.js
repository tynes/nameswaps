...
class InvPacket extends Packet {
  ...
  write(bw) {
    ...
    // TODO: hack, don't need nested varints
    for (const item of this.items)
  ...
  read(br) {
    ...
    for (let i = 0; i < count; i++) {
      ...
      this.items.push(InvItem.decode(data));
    }
...
class DataSyncPacket extends Packet {
  constructor(items, notFound) {
    ...
    this.items = items || new BufferMap();
    this.notFound = notFound || [];
  }

  getSize() {
    let size = 0;
    size += encoding.sizeVarint(this.items.size);
  ...
  read(br) {
    ...
    for (let i = 0; i < resolvedSize; i++) {
      ...
      for (let j = 0; j < size; j++) {
        ...
        items.push(exports.decode(type, packet))
      }

      this.items.set(hash, items);
    }
  ...
  write(bw) {
    bw.writeVarint(this.items.size);
    for (const [hash, items] of this.items) {
      bw.writeHash(hash)
      bw.writeVarint(items.length);
