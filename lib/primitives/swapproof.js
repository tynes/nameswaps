...
class SwapProof extends bio.Struct {
  ...
  fromOptions(options) {
    ...
    if (options.signature) {
      ...
      assert(options.signature.length === 65);
  ...
  isNull() {
    return this.name === '' && this.program.isNull();
  }

  // Offsets are hardcoded and will need to be updated
  // if the script changes. That is why there are versions.
  getPublicKey() {
    if (this.version === 0) {
      const publicKey = this.program.data.slice(23, 56);
  ...
  getSize() {
    ...
    size += this.program.outpoint.getSize();
  ...
  // since the outpoint is the key of the program,
  // we don't want to depend on a race condition that
  // the program must exist beforehand, so therefore
  // place the outpoint in the binary serialization.
  read(br, network) {
    this.version = br.readU8();
    this.name = br.readVarString();
    const hash = br.readHash();
    const index = br.readU32();
    this.program = Program.decode(br.readVarBytes(), {hash, index});
  ...
  // TODO: could refactor out the name, it is
  // the key in the db, so it could be the second
  // arg to write, its needed for p2p serialization
  write(bw) {
    bw.writeU8(this.version);
    bw.writeVarString(this.name);

    bw.writeHash(this.program.outpoint.hash);
    bw.writeU32(this.program.outpoint.index);
